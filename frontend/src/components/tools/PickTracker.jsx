...
export function openAddPick(prefill = {}) {
  ...
  try {
    ...
    const newPick = {
      ...
      date: prefill.date || new Date().toLocaleDateString("en-CA"),
...
export default function PickTracker() {
  ...
  // Load picks from Supabase on mount / user change, sync to localStorage
  useEffect(() => {
    let cancelled = false;
    async function syncFromDb() {
      ...
      if (cancelled) return;
      if (dbPicks.length > 0) {
        // Normalize DB rows to match local format
    ...
    for (const date of datesToResolve) {
      ...
      try {
        ...
        for (const pick of byDate[date]) {
          ...
          // Method 1: direct playerId match using specific gamePk first (critical for doubleheaders)
          ...
          // Method 1b: playerId match across any game that day (if no gamePk stored)
          if (!stats && pick.playerId && allPlayerStats[pick.playerId]) {
            stats = allPlayerStats[pick.playerId];
          }

          // Method 2: name search fallback — normalize names to handle accents/punctuation
          if (!stats && pick.playerName) {
            const normalize = s => s.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "").replace(/[^a-z ]/g, "").trim();
            const target = normalize(pick.playerName);
            // Try exact normalized match first across all player IDs in today's boxes
            for (const [, gData] of Object.entries(boxDataByGame)) {
              if (!gData) continue;
              for (const [pid, pStats] of Object.entries(gData)) {
                if (pStats.name && normalize(pStats.name) === target) {
                  stats = pStats;
  ...
  function addPick(pick) {
    const newPick = { ...pick, id: `${Date.now()}-${Math.random()}`, result: "pending", autoResolved: false, gamePk: pick.gamePk || null };
  ...
  return (
    <div>
      ...
      <div className="record-strip">
        <div className="record-card" style={{ cursor: "pointer" }} onClick={() => setFilterResult("all")}>
        ...
        <div className="record-card" style={{ cursor: "pointer" }} onClick={() => setFilterResult("hit")}>
          <div className="record-card-label">Wins</div>
          <div className="record-card-val" style={{ color: wins > 0 ? "var(--data-green-light)" : "var(--text-muted)" }}>
            {wins}
          </div>
          <div className="record-card-sub">{winRate !== null ? `${winRate}% win rate` : "—"}</div>
        </div>

        <div className="record-card" style={{ cursor: "pointer" }} onClick={() => setFilterResult("miss")}>
          <div className="record-card-label">Losses</div>
          <div className="record-card-val" style={{ color: losses > 0 ? "var(--data-red)" : "var(--text-muted)" }}>
            {losses}
          </div>
        ...
        <div className="record-card" style={{ cursor: "pointer" }} onClick={() => setFilterResult("all")}>
          ...
          <div className="record-card-sub">{pending} pending · {streak > 0 ? `${streak}${streakType === "hit" ? "W" : "L"} streak` : "no streak"}</div>
        </div>
      </div>

      {/* Streak history chart */}
      {decided.length >= 3 && (() => {
        // Group decided picks by date, compute daily hit rate
        const byDateMap = {};
        for (const p of decided) {
          if (!byDateMap[p.date]) byDateMap[p.date] = { hits: 0, total: 0 };
          byDateMap[p.date].total++;
          if (p.result === "hit") byDateMap[p.date].hits++;
        }
        const datePts = Object.entries(byDateMap)
          .sort(([a], [b]) => a.localeCompare(b))
          .map(([date, d]) => ({ date, rate: d.hits / d.total, hits: d.hits, total: d.total }));
        if (datePts.length < 2) return null;

        const W = 100, H = 40;
        const pts = datePts.map((d, i) => ({
          x: (i / (datePts.length - 1)) * W,
          y: H - d.rate * H,
          ...d,
        }));
              ...
              {pts.map((p, i) => (
                <g key={i}>
                  <circle cx={p.x} cy={p.y} r="2.5"
                    fill={p.rate >= 0.5 ? "var(--accent)" : "var(--data-red)"}
                        ...
                        onClick={async (e) => {
                          ...
                          if (p.playerId) {
                            try {
                              ...
                              setPlayerStats({ l7, l3, streak, season, gamelog: gamelog.slice(0, 7) });
                    ...
                    <td>
                      {p.result === "hit" ? (
                        <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
                          ...
                          {p.autoResolved && <span className="material-icons" style={{ fontSize: 10, color: "var(--text-muted)" }} title="Auto-resolved from box score">smart_toy</span>}
                        </div>
                      ) : p.result === "miss" ? (
                        <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
                        ...
                        <span className="badge badge-gray" style={{ fontSize: 10 }}>
                          <span className="material-icons" style={{ fontSize: 11 }}>schedule</span> Pending
                        </span>
                      )}
                    </td>
                    <td>
