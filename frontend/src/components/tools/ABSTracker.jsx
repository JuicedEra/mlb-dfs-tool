...
// Fetch season-level ABS leaderboard from Baseball Savant CSV
async function fetchSavantABSLeaderboard(type = "batter") {
  // Baseball Savant exposes ABS data at /abs with CSV export
  // type: batter | pitcher | catcher | umpire | team
  try {
    const url = `/savant-proxy/abs?season=${SEASON}&type=${type}&format=csv`;
...
export default function ABSTracker() {
  ...
  useEffect(() => { if (!seasonMode) load(); }, [date, seasonMode]);

  useEffect(() => {
    if (seasonMode && !seasonData) loadSeasonData();
  }, [seasonMode]);
  ...
  async function loadSeasonData() {
    ...
    // Load last N days of game-by-game data since Savant CSV proxy may not be available
    try {
      ...
      const results = await Promise.allSettled(days.map(d => fetchABSForDate(d)));
      const allGames = results.filter(r => r.status === "fulfilled").flatMap(r => r.value || []);
      setSeasonData(allGames);
    } catch (e) { console.error(e); setSeasonData([]); }
  ...
  // Catcher board — catchers can challenge as batter (offense) or on behalf of pitcher (defense)
  const catcherBoard = (() => {
    const map = {};
    for (const c of allChallenges) {
      // Check if the challenger is a catcher by position
      const isCatcherBatting = c.challengerSide === "offense" && c.batter?.position === "C";
      const isCatcherDefending = c.challengerSide === "defense" && c.catcher;
      if (isCatcherBatting || isCatcherDefending) {
        const p = isCatcherBatting ? c.batter : c.catcher;
        if (!p?.id && !p?.name) continue;
        const uid = p.id || p.name;
        if (!map[uid]) map[uid] = { id: p.id, name: p.name, total: 0, won: 0, lost: 0, unknown: 0, asBatter: 0, asDefense: 0, challenges: [] };
        map[uid].total++;
        if (isCatcherBatting) map[uid].asBatter++;
        else map[uid].asDefense++;
        if (c.challengeResult === "overturned") map[uid].won++;
        else if (c.challengeResult === "upheld") map[uid].lost++;
        else map[uid].unknown++;
        map[uid].challenges.push(c);
  ...
  // Team board — aggregate by team
  const teamBoard = (() => {
    const map = {};
    for (const c of allChallenges) {
      const team = c.challengerSide === "offense" ? c.battingTeam : c.fieldingTeam;
      if (!team) continue;
      const tid = team.abbr || team.name || "?";
      if (!map[tid]) map[tid] = { name: tid, total: 0, won: 0, lost: 0, unknown: 0, challenges: [] };
      map[tid].total++;
      if (c.challengeResult === "overturned") map[tid].won++;
      else if (c.challengeResult === "upheld") map[tid].lost++;
      else map[tid].unknown++;
      map[tid].challenges.push(c);
    }
    return Object.values(map).filter(t => t.total > 0).sort((a, b) => b.total - a.total);
  })();
    ...
    <div>
      <div className="page-header">
        <div className="page-title-group">
          ...
          <p className="page-subtitle">{seasonMode ? `${SEASON} season leaderboard — last 14 days of game data` : "Challenge data by batters, pitchers, catchers, umpires and teams"}</p>
        </div>
        <div className="page-actions">
          <div style={{ display: "flex", gap: 6 }}>
            <button className={`btn btn-sm ${!seasonMode ? "btn-primary" : ""}`} onClick={() => setSeasonMode(false)}
              style={!seasonMode ? {} : { background: "var(--surface-2)", border: "1px solid var(--border)", color: "var(--text-secondary)" }}>
              <span className="material-icons">today</span> Daily
            </button>
            <button className={`btn btn-sm ${seasonMode ? "btn-primary" : ""}`} onClick={() => setSeasonMode(true)}
              style={seasonMode ? {} : { background: "var(--surface-2)", border: "1px solid var(--border)", color: "var(--text-secondary)" }}>
              <span className="material-icons">leaderboard</span> Season
            </button>
          </div>
          {!seasonMode && (
            <>
          ...
          {seasonMode && (
            <button className="btn btn-primary btn-sm" onClick={loadSeasonData} disabled={seasonLoading}>
              <span className="material-icons">refresh</span>{seasonLoading ? "Loading..." : "Refresh"}
            </button>
          )}
        </div>
      </div>
      ...
      {seasonMode && (() => {
        if (seasonLoading) return <div className="loading"><div className="spinner" />Loading season data...</div>;
        if (!seasonData || seasonData.length === 0) return (
          <div className="empty-state">
            <div className="empty-icon"><span className="material-icons">sports_baseball</span></div>
            <div className="empty-title">No season data yet</div>
            <div className="empty-sub">Season data accumulates as games are played. Check back once the regular season begins.</div>
          </div>
        );
        // Build season leaderboards from accumulated game data
        const allSznChallenges = seasonData.flatMap(g =>
          g.challenges.map(c => ({ ...c, homeAbbr: g.homeAbbr, awayAbbr: g.awayAbbr, gamePk: g.gamePk }))
        );
        function buildSznBoard(key) {
          const map = {};
          for (const c of allSznChallenges) {
            const p = key === "umpire" ? (c.gameUmpire || c.umpire) : c[key];
            if (!p || (!p.id && !p.name)) continue;
            const uid = p.id || p.name;
            const isMine = key === "batter" ? c.challengerSide === "offense"
              : key === "pitcher" ? c.challengerSide === "defense"
              : true;
            if ((key === "batter" || key === "pitcher") && !isMine) continue;
            if (!map[uid]) map[uid] = { id: p.id, name: p.name, total: 0, won: 0, lost: 0 };
            map[uid].total++;
            if (key === "umpire") {
              if (c.challengeResult === "upheld") map[uid].won++;
              else if (c.challengeResult === "overturned") map[uid].lost++;
          ...
          return Object.values(map).filter(p => p.total >= 2)
            .sort((a, b) => b.won / (b.total || 1) - a.won / (a.total || 1));
        }
        const TABS = [
          { key: "batter", label: "Batters", icon: "sports_baseball", desc: "Challenge success rate as batter" },
          { key: "pitcher", label: "Pitchers/Catchers", icon: "person", desc: "Challenge success rate on defense" },
          { key: "umpire", label: "Umpires", icon: "gavel", desc: "Calls upheld (higher = more accurate ump)" },
        ];
        const [activeTab, setActiveTab] = [view, setView];
        const board = buildSznBoard(activeTab === "umpire" ? "umpire" : activeTab === "pitcher" ? "pitcher" : "batter");
        const sznTotal = allSznChallenges.length;
        const sznOvr   = allSznChallenges.filter(c => c.challengeResult === "overturned").length;
        return (
          <div>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(120px, 1fr))", gap: 10, marginBottom: 18 }}>
              <StatCard label="Total Challenges" value={sznTotal} icon="gavel" />
              <StatCard label="Overturned" value={sznOvr} icon="check_circle" color="var(--green-light)" />
              <StatCard label="Overturn %" value={sznTotal ? Math.round(sznOvr / sznTotal * 100) + "%" : "—"} icon="percent" color="var(--yellow)" />
              <StatCard label="Games" value={seasonData.length} icon="today" />
            </div>
            <div className="card" style={{ padding: 0 }}>
              <div className="card-header" style={{ gap: 12 }}>
                {TABS.map(t => (
                  <button key={t.key} onClick={() => setView(t.key)}
                    className={`chip ${view === t.key ? "active" : ""}`}
                    style={{ display: "flex", alignItems: "center", gap: 5 }}>
                    <span className="material-icons" style={{ fontSize: 13 }}>{t.icon}</span>{t.label}
                  </button>
                ))}
              </div>
              <div className="table-wrap">
                <table>
                  <thead><tr>
                    <th>#</th>
                    <th>{view === "umpire" ? "Umpire" : "Player"}</th>
                    <th data-tooltip="Total challenges made">Challenges</th>
                    <th data-tooltip={view === "umpire" ? "Calls upheld by ABS" : "Challenges successfully overturned"}>Won</th>
                    <th data-tooltip={view === "umpire" ? "Calls overturned (misses)" : "Challenges failed"}>Lost</th>
                    <th data-tooltip={view === "umpire" ? "% of calls upheld" : "% of challenges won"}>Success %</th>
                  </tr></thead>
                  <tbody>
                    {board.slice(0, 30).map((p, i) => {
                      const pct = p.total > 0 ? Math.round(p.won / p.total * 100) : 0;
                      return (
                        <tr key={p.id || p.name}>
                          <td style={{ color: "var(--text-muted)", fontSize: 11 }}>{i + 1}</td>
                          <td style={{ display: "flex", alignItems: "center", gap: 8 }}>
                            {p.id && view !== "umpire" && (
                              <img src={headshot(p.id)} alt="" style={{ width: 26, height: 26, borderRadius: "50%", objectFit: "cover" }} onError={e => { e.target.style.display = "none"; }} />
                            )}
                            <span className="td-player">{p.name}</span>
                          </td>
                          <td>{p.total}</td>
                          <td style={{ color: "var(--green-light)", fontWeight: 700 }}>{p.won}</td>
                          <td style={{ color: "var(--red-data)" }}>{p.lost}</td>
                          <td>
                            <span className={`badge ${pct >= 60 ? "badge-green" : pct >= 40 ? "badge-gray" : "badge-yellow"}`}>{pct}%</span>
                          </td>
                        </tr>
                      );
                    })}
                    {board.length === 0 && (
                      <tr><td colSpan={6} style={{ textAlign: "center", color: "var(--text-muted)", padding: 24 }}>No data with 2+ challenges yet</td></tr>
                    )}
                  </tbody>
                </table>
              </div>
      ...
      {seasonMode && null /* skip daily view when in season mode */}

      {!seasonMode && !loading && total > 0 && (
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(120px, 1fr))", gap: 10, marginBottom: 18 }}>
      ...
      <div className="filter-bar" style={{ marginBottom: 16 }}>
        <div className="chip-group">
          {[["games","By Game"],["batters","Batters"],["pitchers","Pitchers"],["catchers","Catchers"],["teams","Teams"],["umpires","Umpires"]].map(([id, label]) => (
      ...
      {!seasonMode && !loading && view === "games" && total > 0 && (
      ...
      {!seasonMode && !loading && view === "batters" && <LeaderboardTable title="Batter Challenges" subtitle="Challenges initiated by batters" icon="sports_baseball" board={batterBoard} roleLabel="Batter" />}
      {!seasonMode && !loading && view === "pitchers" && <LeaderboardTable title="Pitcher / Catcher Challenges" subtitle="Challenges initiated by the defensive side" icon="shield" board={pitcherBoard} roleLabel="Pitcher" />}
      {!seasonMode && !loading && view === "catchers" && (
        <div className="card" style={{ marginTop: 16 }}>
          <div className="card-header"><span className="card-title"><span className="material-icons">sports</span>Catcher Challenges</span></div>
          <div className="card-sub" style={{ padding: "0 16px 8px", fontSize: 11, color: "var(--text-muted)" }}>
            Catchers who challenged — as batter (offense) or on behalf of pitcher (defense)
          </div>
          <table style={{ fontSize: 12, width: "100%" }}>
            <thead><tr><th>#</th><th>Catcher</th><th>Total</th><th>Won</th><th>Lost</th><th title="Challenges while batting">As Batter</th><th title="Challenges on behalf of pitcher">As Defense</th><th>Win %</th></tr></thead>
            <tbody>
              {catcherBoard.length > 0 ? catcherBoard.map((c, i) => (
                <tr key={c.id || i}>
                  <td className="td-mono" style={{ color: "var(--text-muted)" }}>{i+1}</td>
                  <td className="td-player">{c.name}</td>
                  <td className="td-mono" style={{ fontWeight: 700 }}>{c.total}</td>
                  <td className="td-mono" style={{ color: "var(--green-light)" }}>{c.won}</td>
                  <td className="td-mono" style={{ color: "var(--red-data)" }}>{c.lost}</td>
                  <td className="td-mono">{c.asBatter}</td>
                  <td className="td-mono">{c.asDefense}</td>
                  <td className="td-mono" style={{ fontWeight: 700 }}>{c.total ? Math.round((c.won / c.total) * 100) + "%" : "—"}</td>
                </tr>
              )) : <tr><td colSpan="8" style={{ textAlign: "center", padding: 20, color: "var(--text-muted)" }}>No catcher challenges found</td></tr>}
            </tbody>
          </table>
        </div>
      )}
      {!seasonMode && !loading && view === "teams" && (
        <div className="card" style={{ marginTop: 16 }}>
          <div className="card-header"><span className="card-title"><span className="material-icons">groups</span>Team Challenges</span></div>
          <div className="card-sub" style={{ padding: "0 16px 8px", fontSize: 11, color: "var(--text-muted)" }}>
            Total ABS challenges by team — successful vs unsuccessful
          </div>
          <table style={{ fontSize: 12, width: "100%" }}>
            <thead><tr><th>#</th><th>Team</th><th>Total</th><th>Won</th><th>Lost</th><th>Win %</th><th>Success Rate</th></tr></thead>
            <tbody>
              {teamBoard.length > 0 ? teamBoard.map((t, i) => {
                const pct = t.total ? Math.round((t.won / t.total) * 100) : 0;
                return (
                  <tr key={t.name}>
                    <td className="td-mono" style={{ color: "var(--text-muted)" }}>{i+1}</td>
                    <td>
                    ...
                    <td className="td-mono" style={{ fontWeight: 700 }}>{t.total}</td>
                    <td className="td-mono" style={{ color: "var(--green-light)" }}>{t.won}</td>
                    <td className="td-mono" style={{ color: "var(--red-data)" }}>{t.lost}</td>
                    <td className="td-mono" style={{ fontWeight: 700 }}>{pct}%</td>
                    <td>
                      <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                        <div style={{ flex: 1, height: 6, background: "var(--border)", borderRadius: 100, overflow: "hidden" }}>
                          <div style={{ height: "100%", width: `${pct}%`, background: pct >= 50 ? "var(--green-light)" : "var(--red-data)", borderRadius: 100 }} />
                        </div>
                      </div>
                    </td>
                  </tr>
                );
              }) : <tr><td colSpan="7" style={{ textAlign: "center", padding: 20, color: "var(--text-muted)" }}>No team data found</td></tr>}
            </tbody>
          </table>
        </div>
      )}
      {!seasonMode && !loading && view === "umpires" && <LeaderboardTable title="Umpire Scorecard" subtitle="Lower overturn % = better accuracy" icon="visibility" board={umpireBoard} roleLabel="Umpire" isUmpire />}
