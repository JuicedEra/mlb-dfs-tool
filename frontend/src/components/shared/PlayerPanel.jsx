...
export default function PlayerPanel({ pick, onClose, showBvP = true, liveStats }) {
  const { batter, pitcher, game, scoreData } = pick;
  // Live box score stats for this player's game
  const liveGameData  = liveStats?.[game?.gamePk];
  const livePlayerStat = liveGameData?.stats?.get(batter.id);
  const gameActualStatus = liveGameData?.status; // "Live" | "Final" | "Preview"
      ...
      <div className="side-panel">
        ...
        <div className="side-panel-header">
          <div style={{ flex: 1 }}>
            ...
            <button
              onClick={() => window.dispatchEvent(new CustomEvent("diamondiq:trackpick", { detail: pick }))}
        ...
        <div className="side-panel-body">
          {loading ? (
            ...
            <>
              {activeTab === "overview" && (
                <>
                  {/* Live / Final game stat line */}
                  {livePlayerStat && (gameActualStatus === "Live" || gameActualStatus === "Final") && (
                    <div style={{
                      marginBottom: 16, padding: "12px 16px", borderRadius: 10,
                      background: gameActualStatus === "Final" ? "var(--surface-2)" : "linear-gradient(135deg, rgba(10,110,75,0.10), rgba(0,45,114,0.10))",
                      border: `1px solid ${gameActualStatus === "Final" ? "var(--border)" : "rgba(10,110,75,0.25)"}`,
                      display: "flex", alignItems: "center", justifyContent: "space-between", gap: 16,
                    }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                        <span className="material-icons" style={{ fontSize: 16, color: gameActualStatus === "Final" ? "var(--text-muted)" : "var(--accent-light)" }}>
                          {gameActualStatus === "Final" ? "check_circle" : "sports_baseball"}
                        </span>
                        <span style={{ fontSize: 11, fontWeight: 800, color: gameActualStatus === "Final" ? "var(--text-muted)" : "var(--accent-light)", textTransform: "uppercase", letterSpacing: "0.8px" }}>
                          {gameActualStatus === "Final" ? "Final" : "Live"}
                        </span>
                      </div>
                      <div style={{ display: "flex", gap: 16, alignItems: "center" }}>
                        <div style={{ textAlign: "center" }}>
                          <div style={{ fontFamily: "var(--font-mono)", fontSize: 22, fontWeight: 900, color: livePlayerStat.hits > 0 ? "var(--data-green)" : "var(--text-primary)", lineHeight: 1 }}>
                            {livePlayerStat.hits}-{livePlayerStat.atBats}
                          </div>
                          <div style={{ fontSize: 9, color: "var(--text-muted)", textTransform: "uppercase", letterSpacing: "0.5px", marginTop: 2 }}>H-AB</div>
                        </div>
                        <div style={{ textAlign: "center" }}>
                          <div style={{ fontFamily: "var(--font-mono)", fontSize: 22, fontWeight: 900, color: livePlayerStat.totalBases > 0 ? "var(--navy)" : "var(--text-secondary)", lineHeight: 1 }}>
                            {livePlayerStat.totalBases}
                          </div>
                          <div style={{ fontSize: 9, color: "var(--text-muted)", textTransform: "uppercase", letterSpacing: "0.5px", marginTop: 2 }}>TB</div>
                        </div>
                        <div style={{ textAlign: "center" }}>
                          <div style={{ fontFamily: "var(--font-mono)", fontSize: 22, fontWeight: 900, color: livePlayerStat.rbi > 0 ? "var(--data-yellow)" : "var(--text-secondary)", lineHeight: 1 }}>
                            {livePlayerStat.rbi}
                          </div>
                          <div style={{ fontSize: 9, color: "var(--text-muted)", textTransform: "uppercase", letterSpacing: "0.5px", marginTop: 2 }}>RBI</div>
                        </div>
                      </div>
                    </div>
                  )}
